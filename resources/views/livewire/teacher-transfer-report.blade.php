<div>
    ...
    <form wire:submit.prevent="exportExcel" enctype="multipart/form-data">
        ...
        <div class="w-full grid grid-cols-1 gap-y-1 sm:grid-cols-4">
            ...
            <div class="sm:col-span-1 px-1">
                <x-form-input-label for="transferType" value="Transfer Type" />
                <x-form-list-input-field name="transferType" id="transferType" :options="$transferTypeList" wire:model.live="transferType"/>
            </div>
            <div class="sm:col-span-1 px-1">
        ...
        <div class="flex items-center justify-start gap-2 mx-4">
            ...
            <a href="{{ route('teacher.transferreport') }}" class="mt-4 p-2 transition duration-200 bg-white hover:bg-white focus:bg-white
            ...
            <div class="p-1.5 min-w-full inline-block align-middle">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">{{ $result->userService->user->nic }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">{{ $result->userService->user->nameWithInitials }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">{{ $result->userService->user->currentTeacherService?->currentAppointment?->workPlace?->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">
                            @if($result->userService->user->personalInfo?->genderId == 1)
                                Male
                            @elseif($result->userService->user->personalInfo?->genderId == 2)
                                Female
                            @else
                                N/A
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">{{ $result->userService->user->currentTeacherService?->appointedDate }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-white">{{ $result->userService->user->currentTeacherService?->currentAppointment?->appointedDate }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
