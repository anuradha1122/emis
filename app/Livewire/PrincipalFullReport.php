...
use App\Exports\PrincipalFullReportExport;


class PrincipalFullReport extends Component
{
    use WithPagination;
    ...
    private function getQuery()
    {
        ...
        if ($this->serviceStart) {
            $query->whereHas('currentPrincipalService', fn($q) => $q->where('appointedDate', '>=', $this->serviceStart));
            $this->reportAction = null;
        }
        if ($this->serviceEnd) {
            $query->whereHas('currentPrincipalService', fn($q) => $q->where('appointedDate', '<=', $this->serviceEnd));
            $this->reportAction = null;
        }
    ...
    public function exportExcel()
    {
        ...
        return Excel::download(new PrincipalFullReportExport($userIds), 'principal_full_report.xlsx');
    }
    ...
    public function render()
    {
        ...
        return view('livewire.principal-full-report', compact('results'));
    }
}
